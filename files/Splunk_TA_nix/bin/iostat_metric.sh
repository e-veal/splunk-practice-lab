...
if [ "x$KERNEL" = "xLinux" ] ; then
	...
	FORMAT='{device=$1; rReq_PS=$idx_rReq_PS; wReq_PS=$idx_wReq_PS; rKB_PS=$idx_rKB_PS; wKB_PS=$idx_wKB_PS; avgQueueSZ=$idx_avgQueueSZ; if(await==""){avgWaitMillis="?";} else {avgWaitMillis=$await;}; if(svctm==""){avgSvcMillis="?";} else {avgSvcMillis=$svctm;}; bandwUtilPct=$putil;OSName=OSName;OS_version=OS_version;IP_address=IP_address;}'
